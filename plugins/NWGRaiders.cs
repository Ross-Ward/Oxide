...
{
    [Info("NWGRaiders", "New World Gaming", "1.0.0")]
    [Description("Spawn Npc to raid your base.")]
    public class NWGRaiders : RustPlugin
    {
        #region Loading
        public static bool debug = false;

        [PluginReference]
        private Plugin NWGKits, TruePVE, ServerRewards, Economics, LifeSupport, ZoneManager;
        ...
        private static string usePerm = "nwgraiders.use";
        private static string coolPerm = "nwgraiders.nocooldown";
        private static string adminPerm = "nwgraiders.admin";
        private static string bypassPerm = "nwgraiders.bypass";
        private static string bypassCost = "nwgraiders.nocost";
        ...
        private void Init()
        {
            ...
            cmd.AddConsoleCommand("raiders", this, "ConsolStartRaid");
        ...
        private void OnServerInitialized()
        {
            ...
            if (configData.raidSettings.raidTypes.Count <= 0)
            {
                ...
                configData.raidSettings.raidTypes.Add("easy", new raidConfig(10, true, 100, 640, new List<string>(), new List<string>() { "explosive.timed", "explosive.satchel" }, 2, "nwgraiders.easy", 15000, 1.0));
                configData.raidSettings.raidTypes.Add("medium", new raidConfig(15, true, 200, 900, new List<string>(), new List<string>() { "explosive.timed", "explosive.satchel" }, 3, "nwgraiders.medium", 30000, 1.0));
                configData.raidSettings.raidTypes.Add("hard", new raidConfig(20, true, 400, 1200, new List<string>(), new List<string>() { "explosive.timed", "explosive.satchel" }, 4, "nwgraiders.hard", 50000, 1.0));
                configData.raidSettings.raidTypes.Add("expert", new raidConfig(30, true, 400, 1500, new List<string>(), new List<string>() { "explosive.timed", "explosive.satchel" }, 4, "nwgraiders.expert", 65000, 1.0));
                configData.raidSettings.raidTypes.Add("nightmare", new raidConfig(30, true, 400, 2100, new List<string>(), new List<string>() { "explosive.timed", "explosive.satchel" }, 5, "nwgraiders.nightmare", 100000, 1.0));
                SaveConfig();
            }
            ...
            if (configData.raidVipOptions == null)
            {
                ...
                configData.raidVipOptions.VipOptions.Add("nwgraiders.vip1", new raidVip(3600, 43200));
                configData.raidVipOptions.VipOptions.Add("nwgraiders.vip2", new raidVip(3600, 43200));
                configData.raidVipOptions.VipOptions.Add("nwgraiders.vip3", new raidVip(3600, 43200));
                SaveConfig();
            }
        ...
        class ConfigData
        {
            ...
            public global::Oxide.Core.VersionNumber Version { get; set; }
        }
        ...
        protected override void LoadConfig()
        {
            ...
            NormalizeLegacyPermissionsInConfig();
        ...
        private bool NormalizeLegacyPermissionsInConfig()
        {
            var changed = false;

            if (configData?.raidSettings?.raidTypes != null)
            {
                foreach (var raidType in configData.raidSettings.raidTypes.Values)
                {
                    var normalized = NormalizeRaidersPermission(raidType.Permission);
                    if (!string.Equals(normalized, raidType.Permission, StringComparison.Ordinal))
                    {
                        raidType.Permission = normalized;
                        changed = true;
                    }
                }
            }

            if (configData?.raidVipOptions?.VipOptions != null && configData.raidVipOptions.VipOptions.Count > 0)
            {
                var rebuilt = new Dictionary<string, raidVip>();
                foreach (var pair in configData.raidVipOptions.VipOptions)
                {
                    var normalizedKey = NormalizeRaidersPermission(pair.Key);
                    if (!string.Equals(normalizedKey, pair.Key, StringComparison.Ordinal))
                        changed = true;

                    if (!rebuilt.ContainsKey(normalizedKey))
                        rebuilt[normalizedKey] = pair.Value;
                }

                configData.raidVipOptions.VipOptions = rebuilt;
            }

            return changed;
        }

        private static string NormalizeRaidersPermission(string permissionName)
        {
            if (string.IsNullOrEmpty(permissionName))
                return permissionName;

            if (permissionName.StartsWith("nwgraiders.", StringComparison.OrdinalIgnoreCase))
                return permissionName;

            if (permissionName.StartsWith("nwg.raiders.", StringComparison.OrdinalIgnoreCase))
                return "nwgraiders." + permissionName.Substring("nwg.raiders.".Length);

            if (permissionName.StartsWith("raiders.", StringComparison.OrdinalIgnoreCase))
                return "nwgraiders." + permissionName.Substring("raiders.".Length);
            ...
            internal void Start()
            {
                ...
                if (building != null)
                    ...
                    { 
                        ...
                        if (config.canNotRepair && !_AllPlayers.Contains(user))
                            _AllPlayers.Add(user);
...
float ProjectileDistToGravity(float x, float y, float θ, float v)
{
    ...
    return num2;
                        ...
                        foreach (var RewardPlayer in onlinePlayer.ToList())
                        {
                            ...
                            else if (reward.RewardType.ToLower() == "kits" || reward.RewardType.ToLower() == "kit")
                            {
                                ...
                                if (totals is string)
                                {
                                    ...
                                    {
                                        object success = _.NWGKits?.Call("GiveKit", RewardPlayer, Convert.ToString(totals));
                                        if (success == null)
                                ...
                                else if (kitList != null)
                                {
                                    ...
                                    object success = _.NWGKits?.Call("GiveKit", RewardPlayer, rand);
                                    if (success == null)
                        ...
                        if (player != null)
                        {
                            ...
                            else if (reward.RewardType.ToLower() == "kits" || reward.RewardType.ToLower() == "kit")
                            {
                                ...
                                if (totals is string)
                                {
                                    if (!string.IsNullOrEmpty(Convert.ToString(totals)))
                                    {
                                        object success = _.NWGKits?.Call("GiveKit", player, Convert.ToString(totals));
                                        if (success == null)
                                            _.PrintWarning($"Unable to find a kit with the name {Convert.ToString(totals)}");
                                        else if (success is string)
                                        {
                                ...
                                else if (kitList != null && kitList.Count > 0)
                                {
                                    ...
                                    object success = _.NWGKits?.Call("GiveKit", player, rand);
                                    if (success == null)
                                        _.PrintWarning($"Unable to find a kit with the name {rand}");
                                    else if (success is string)
                                    {
        ...
        public class NpcRaider : NPCPlayer, IAISenses, IAIAttack, IThinker
        {
            ...
            public void setUpGear(string kitname = "")
            {
                ...
                if (!string.IsNullOrEmpty(kitname))
                {
                    ...
                    object success = _.NWGKits?.Call("GetKitInfo", kitname);
                    if (success == null)
                    {
                        addDefaultGear();
            ...
            else
            {
                if (canBuyRaid(player, type))
                {
                    ...
                    foreach (var user in priv.authorizedPlayers.ToList())
                    {
                        ...
                        {
                            foreach (var perm in configData.raidVipOptions.VipOptions)
                                if (permission.UserHasPermission(user.ToString(), perm.Key))
                                    cooldownTime = perm.Value.cooldownSeconds;
                        }
        ...
        public static void startRaidForPlayer(BasePlayer player, Vector3 loc, BuildingPrivlidge priv, string type)
        {
            ...
            if (priv != null)
            {
                foreach (var user in priv.authorizedPlayers.ToList())
                {
                    if (user == player.userID)
                    {
                        if (!_.statsData.pEntity.ContainsKey(user))
                            _.statsData.pEntity.Add(user, new StatsInfo() { name = player.displayName });
                        _.statsData.pEntity[user].played++;
                        continue;
                    }
                    BasePlayer TCplayer = BasePlayer.FindByID(user);
                    if (TCplayer != null)
                    {
                        ...
                        if (!_.statsData.pEntity.ContainsKey(user))
                            _.statsData.pEntity.Add(user, new StatsInfo() { name = TCplayer.displayName });
                        _.statsData.pEntity[user].played++;
                    }
                }
                _.SaveStats();
        ...
        private new void LoadDefaultMessages()
        {
            ...
            {
                ["blocked"] = "<color=#9fb8ad>You need to be in a building area!</color>",
                ["start"] = "<color=#9fb8ad>Your event is starting!</color>",
                ["nextWave"] = "<color=#9fb8ad>The next wave of raiders are coming.</color>",
                ["eventOver"] = "<color=#9fb8ad>Your event is now over!</color>",
                ["eventOverWin"] = "<color=#9fb8ad>Your just won the event!</color>",
                ["waiting"] = "<color=#9fb8ad>Your event will start shortly!</color>",
                ["noHurt"] = "You can not hurt the event npc!   ",
                ["max"] = "<color=#9fb8ad>The maximum amount of event's are running. There is {0} ahead of you.</color>",
                ["noTypeRaid"] = "<color=#9fb8ad>There is no raid type named {0}</color>",
                ["cooldown"] = "<color=#9fb8ad>You are in cooldown for {0} seconds!</color>",
                ["cooldownAuth"] = "<color=#9fb8ad>You are in cooldown for Cupboard Authorize {0} seconds!</color>",
                ["ThereAlready"] = "<color=#9fb8ad>You are already wating on the event.</color>",
                ["InEvent"] = "<color=#9fb8ad>Someone authed on the TC just started a raid event.</color>",
                ["noPerm"] = "<color=#9fb8ad>You do not have permission to use this raid type!</color>",
                ["noPermUse"] = "<color=#9fb8ad>You do not have permission to execute this command!</color>",
                ["noMoney"] = "<color=#9fb8ad>You need {0} {1} to start this event!</color>",
                ["MultiplePlayers"] = "<color=#9fb8ad>Found to many matches {0}!</color>",
                ["destroyEvent"] = "<color=#9fb8ad>Event for {0} destroyed!</color>",
                ["destroyEventNotFound"] = "<color=#9fb8ad>Event for {0} was not found!</color>",
                ["MoneyIssued"] = "<color=#9fb8ad>{1} you just recived ${0} for winning the event.</color>",
                ["Blockedhere"] = "<color=#9fb8ad>The Admin has blocked raids on this terrain type.</color>",
                ["BlockedhereWater"] = "<color=#9fb8ad>You can not start the event underground.</color>",
                ["RewardedItem"] = "<color=#9fb8ad>You just won the event and got {0} {1}</color>",
                ["waitingNext"] = "<color=#9fb8ad>There is currently {0} events running you are next in line!</color>",
                ["waitingLine"] = "<color=#9fb8ad>There is currently {0} events running and {1} ahead of you. You will be notified before your event starts.</color>",
                ["waitingToStart"] = "<color=#9fb8ad>Your event will start in 60 seconds.</color>",
                ["TcNotFound"] = "<color=#9fb8ad>Your event tc was not found, Event canceled!</color>",
                ["NoLoot"] = "<color=#9fb8ad>You can not loot this event corpse!</color>",
                ["UIHeader"] = "<color=#9fb8ad>NPC RAIDERS STATS</color>",
                ["UI1"] = "<color=#9fb8ad>Total events played</color><color=white>:</color>",
                ["UI2"] = "<color=#9fb8ad>Total events won</color><color=white>:</color>",
                ["UI3"] = "<color=#9fb8ad>Total events lost</color><color=white>:</color>",
                ["UI4"] = "<color=#9fb8ad>Total npc's killed</color><color=white>:</color>",
                ["ArivalTimeNext"] = "<color=#9fb8ad>Wave {1}/{0} will start in</color><color=white>:</color><color=yellow>{2}</color>",
                ["NoRepair"] = "You can not repair while in this event!",
                ["EventEndINTime"] = "<color=#9fb8ad>You're event will end in {0}</color>",
                ["ChatHeader"] = "<size=18><color=#9fb8ad>[ NWG ]</color> Raiders</size>",
                ["Reward"] = "Reward <color=#9fb8ad>$"
            }, this);
        }
        #endregion
