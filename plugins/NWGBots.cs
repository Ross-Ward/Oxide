...
namespace Oxide.Plugins
{
    ...
    [Info("NWGBots", "New World Gaming", "1.0.0")]
    [Description("Spawn tailored AI with kits at monuments, custom locations, or randomly.")]

    public class NWGBots : RustPlugin
    {
        [PluginReference] Plugin Convoy, NoSash, NWGKits, CustomLoot, RustRewards, XPerience;
        private Plugin Kits => NWGKits;
        int no_of_AI;
        bool loaded;
        static NWGBots bs;
        ...
        bool HasPermission(string id, string perm)
        {
            ...
            if (perm.StartsWith("nwgbots.", StringComparison.OrdinalIgnoreCase))
                return permission.UserHasPermission(id, "nwg.bots." + perm.Substring("nwgbots.".Length));

            if (perm.StartsWith("nwg.bots.", StringComparison.OrdinalIgnoreCase))
                return permission.UserHasPermission(id, "nwgbots." + perm.Substring("nwg.bots.".Length));

            return false;
        }

        const string permAllowed = "nwgbots.allowed";
        ...
        {
            if (player == null || player.net?.connection != null || player.gameObject?.name == "NWGBots")
                return;
        ...
        void ImportFiles()
        {
            storedData = Interface.Oxide.DataFileSystem.ReadObject<StoredData>($"NWGBots/{configData.DataPrefix}-CustomProfiles");
            defaultData = Interface.Oxide.DataFileSystem.ReadObject<DefaultData>($"NWGBots/{configData.DataPrefix}-DefaultProfiles");
            ...
            spawnsData = Interface.Oxide.DataFileSystem.ReadObject<SpawnsData>($"NWGBots/{configData.DataPrefix}-SpawnsData");
            templateData = Interface.Oxide.DataFileSystem.ReadObject<TemplateData>("NWGBots/TemplateData");
        }
        ...
        void OnEntityKill(ScientistNPC npc, HitInfo info, bool killed)
        {
            if (npc?.userID != null && NPCPlayers.ContainsKey(npc.userID) && !botInventories.ContainsKey(npc.userID))
            {
                ...
                if (killed)
                {
                    ...
                    Interface.CallHook("OnNWGBotsNPCKilled", npc, bData.profilename, bData.group, info);
                }
        ...
        void OnEntitySpawned(HackableLockedCrate crate)
        {
            ...
            NextTick(() =>
            {
                ...
                if (Interface.CallHook("OnNWGBotsCrateDropped", crate) != null)
                    return;
                    ...
                    if (prefab != null && !string.IsNullOrEmpty(prefab.guid))
                    {
                        ...
                        if (spawnpoint)
                        {
                            ...
                            if (npc)
                            {
                                ...
                                if (bs.NWGKits && profile?.Spawn?.Keep_Default_Loadout == false && (UseOR ? or.Kits.ToList() : profile.Spawn.Kit).Count > 0 && npc?.loadouts != null) ////  ADDED
                ...
                for (int i = 0; i < component.numProjectiles; i++)
                {
                    ...
                    for (int j = 0; j < list.Count; j++)
                    {
                        ...
                        if (entity != held && !entity.EqualNetID(held))
                        {
                            ...
                            if (projectile != null)
                            {
                                var doHitEffects = true;
                                var projectileType = projectile.GetType();
                                var prop = projectileType.GetProperty("doDefaultHitEffects");
                                if (prop != null && prop.PropertyType == typeof(bool))
                                {
                                    doHitEffects = (bool)prop.GetValue(projectile, null);
                                }
                                else
                                {
                                    var field = projectileType.GetField("doDefaultHitEffects");
                                    if (field != null && field.FieldType == typeof(bool))
                                        doHitEffects = (bool)field.GetValue(projectile);
                                }

                                hitInfo.DoHitEffects = doHitEffects;
                            }
        ...
        readonly Dictionary<string, string> Messages = new Dictionary<string, string>()
        {
            ...
            {"nonpc", "No NWGBots npc found directly in front of you." },
        ...
        void BSDocs(BasePlayer player, string docname, string doc)
        {
            ...
            elements.Add(new CuiLabel { Text = { Text = "NWGBots", FontSize = 20, Font = Font, Align = TextAnchor.MiddleCenter }, RectTransform = { AnchorMin = "0.2 0.95", AnchorMax = "0.8 1" } }, mainName);
        ...
        void BSMainUI(BasePlayer player, string tab, string profile = "", string sub = "", int page = 1)
        {
            ...
            {
                MsgUI(player, "NWGBots is not initialised yet.", 3f);
                return;
            }
            ...
            if (tab == string.Empty)
            {
                elements.Add(new CuiButton { Button = { Command = "", Color = configData.UI.ButtonColour }, RectTransform = { AnchorMin = $"0 0.95", AnchorMax = $"0.999 0.99" }, Text = { Text = $"NWGBots Settings", FontSize = 18, Font = Font, Align = TextAnchor.MiddleCenter } }, mainName);
