...
{
    [Info("NWGBetterTC", "New World Gaming", "1.0.0")]
    internal class NWGBetterTC : RustPlugin
    {
        #region Fields
        ...
        private const string permadmin = "nwgbettertc.admin";
        private const string permupgrade = "nwgbettertc.upgrade";
        private const string permupgradenocost = "nwgbettertc.upgrade.nocost";
        private const string permrepair = "nwgbettertc.repair";
        private const string permrepairnocost = "nwgbettertc.repair.nocost";
        private const string permreskin = "nwgbettertc.reskin";
        private const string permreskinnocost = "nwgbettertc.reskin.nocost";
        private const string permwallpaper = "nwgbettertc.wallpaper";
        private const string permwallpapernocost = "nwgbettertc.wallpaper.nocost";
        private const string permlist = "nwgbettertc.authlist";
        private const string permdelauth = "nwgbettertc.deleteauth";
        private const string permtcskin = "nwgbettertc.tcskinchange";
        private const string permtcskindeployed = "nwgbettertc.tcskindeployed";
        private const string permupskin = "nwgbettertc.upskin";
        ...
        private void TCSkinReplace(BuildingPrivlidge tc, BasePlayer player, TCSkin skin){
            ...
            NextTick(() => {
                ...
                foreach (var authorized in tc.authorizedPlayers){
                    Building.authorizedPlayers.Add(authorized);
                }
		...
		private List<ulong> GetAuthPlayers(BuildingPrivlidge cup){
            return cup.authorizedPlayers.Where(player => (HasPermission(player.ToString(), permadmin) && config.adminshow == true) || !HasPermission(player.ToString(), permadmin)).ToList();
        }
        ...
        private void CreateGameTip(BuildingPrivlidge cup, string text, BasePlayer player, string sound, float length = 10f, string red = ""){
            ...
            if (cup != null){
                ...
                foreach (ulong pnid in cup.authorizedPlayers) {
                    BasePlayer foundPlayer = BasePlayer.Find(pnid.ToString());
                    if(foundPlayer != null){
		...
		private void ShowMenuAuthlist(BasePlayer player, BuildingPrivlidge cup, int page = 0){
            ...
            foreach (var entry in PlayersTC){
                var authorizedPlayer = BasePlayer.FindByID(entry) ?? BasePlayer.FindSleeping(entry);
                var authorizedName = authorizedPlayer != null ? authorizedPlayer.displayName : entry.ToString();
				...
				UI.Label(ref container, authlist_3, authorizedName, 13, (showsteamid) ? "0.025 0.40" : "0.025 0.05", "0.8 0.95", "1.00 1.00 1.00 0.9", TextAnchor.MiddleLeft, true);
                
                if (showsteamid) UI.Label(ref container, authlist_3, entry.ToString(), 10, "0.025 0.05", "0.8 0.50", "1.00 1.00 1.00 0.7", TextAnchor.MiddleLeft);
                if (HasPermission(player.UserIDString, permdelauth)) UI.Button(ref container, authlist_3, "0.7 0.2 0.2 0.8", "REMOVE", 10, "0.8 0", "1 0.98", $"SENDCMD REMOVEAUTH {page} {cup} {entry}");
        ...
        private class ConfigData {
            ...
            public Dictionary<string, float> FrequencyUpgrade = new Dictionary<string, float>(){
                    ["nwgbettertc.use"] = 2.0f,
                    ["nwgbettertc.vip"] = 1.0f,
            };
        ...
        private bool NormalizeLegacyPermissionsInConfig() {
            ...
            changed |= NormalizePermissionDictionary(ref config.FrequencyUpgrade);
            changed |= NormalizePermissionDictionary(ref config.FrequencyReskin);
            changed |= NormalizePermissionDictionary(ref config.FrequencyRepair);
            changed |= NormalizePermissionDictionary(ref config.FrequencyWallpaper);
            changed |= NormalizePermissionDictionary(ref config.CostListRepair);

            if (config.itemsList != null) {
                foreach (var item in config.itemsList) {
                    var normalized = NormalizeBetterTcPermission(item.permission);
                    if (!string.Equals(normalized, item.permission, StringComparison.Ordinal)) {
                        item.permission = normalized;
                        changed = true;
                    }
                }
            }

            if (config.itemsWallpaperList != null) {
                foreach (var item in config.itemsWallpaperList) {
                    var normalized = NormalizeBetterTcPermission(item.permission);
                    if (!string.Equals(normalized, item.permission, StringComparison.Ordinal)) {
                        item.permission = normalized;
        ...
        private bool NormalizePermissionDictionary(ref Dictionary<string, float> source) {
            if (source == null || source.Count == 0) return false;

            var changed = false;
            var rebuilt = new Dictionary<string, float>();
            foreach (var entry in source) {
                var normalized = NormalizeBetterTcPermission(entry.Key);
                if (!string.Equals(normalized, entry.Key, StringComparison.Ordinal)) changed = true;
                rebuilt[normalized] = entry.Value;
            }
        ...
        private static string NormalizeBetterTcPermission(string permissionName) {
            if (string.IsNullOrEmpty(permissionName)) return permissionName;

            if (permissionName.StartsWith("nwgbettertc.", StringComparison.OrdinalIgnoreCase)) return permissionName;

            if (permissionName.StartsWith("nwg.bettertc.", StringComparison.OrdinalIgnoreCase))
                return "nwgbettertc." + permissionName.Substring("nwg.bettertc.".Length);

            if (permissionName.StartsWith("bettertc.", StringComparison.OrdinalIgnoreCase))
                return "nwgbettertc." + permissionName.Substring("bettertc.".Length);
        ...
        protected override void LoadDefaultConfig(){
            ...
            config.itemsList.Add(new ItemInfo {
                ...
                skinid = 0,
                color = false,
                    permission = "nwgbettertc.updefault"
            });
            SaveConfig();
        }
        #endregion
