...
{
    [Info("NWGBuriedTreasure", "New World Gaming", "1.0.0")]
    [Description("Treasure maps and buried loot — find maps in loot crates, mark locations, and dig up treasure chests.")]
    class NWGBuriedTreasure : RustPlugin
    {
        #region Load

        [PluginReference] Plugin NWGEconomy;
        ...
        private void Loaded()
        {
            ...
            permission.RegisterPermission("nwgburiedtreasure.admin", this);
        }
        ...
        private class PluginConfig
        {
            ...
            {
                [JsonProperty(PropertyName = "Gold - Enable gold to be sold for NWG Economy currency ? ")] public bool UseNWGEconomy { get; set; }
                [JsonProperty(PropertyName = "Gold - Player will get this many NWG Economy currency when selling 1 gold : ")] public int NWGEconomyGoldExchange { get; set; }
        ...
        Dictionary<string, string> Messages = new Dictionary<string, string>
        {
            ...
            ["helptext4"] = "/sellgold - sell for NWG Economy currency.",
        ...
        private void cmdConsoleBuyMap(ConsoleSystem.Arg arg)
        {
            ...
            if (player != null)
            {
                if (!permission.UserHasPermission(player.UserIDString, "nwgburiedtreasure.admin"))
                {
        ...
        private void SellGold(BasePlayer player, Item item = null)
        {
            ...
            if (activeItem != null)
            {
                ...
                {
                    if (config.globalSettings.UseNWGEconomy && NWGEconomy != null)
                    {
                        NWGEconomy?.Call("Deposit", new object[] { player.userID, Convert.ToDouble(config.globalSettings.NWGEconomyGoldExchange) });
                        SendReply(player, "You sold your gold for " + config.globalSettings.NWGEconomyGoldExchange.ToString() + " coins!");
                        activeItem.Remove(0f);
                    }
                    SendReply(player, "You sold your gold.");
                    activeItem.Remove(0f);
                    return;
                }
            }
        ...
        private class TreasureMarker : BaseEntity
        {
            ...
            private NWGBuriedTreasure instance;
            public int rarity;
            ...
            private void Awake()
            {
                instance = new NWGBuriedTreasure();
