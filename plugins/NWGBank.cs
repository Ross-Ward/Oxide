...
using Oxide.Plugins.NWGBankExtensionMethods;
using UnityEngine;
...
{
	[Info("NWGBank", "New World Gaming", "1.0.0")]
	public class NWGBank : RustPlugin
	{
		#region Fields
		...
		private const string Layer = "NWG.Bank";

		private static NWGBank _instance;
		...
		private readonly List<CardReader> _readers = new List<CardReader>();
		...
		private class Configuration
		{
			...
			public string[] Commands = {"nbank", "nwg.bank"};

			[JsonProperty(PropertyName = "Permission (example: nwgbank.use)")]
			...
			public ATMSettings Atm = new ATMSettings
			{
				...
				PermissionToOpenATMviaCommand = "nwgbank.openatm",
		...
		private class ATMSettings
		{
			...
			[JsonProperty(PropertyName = "Use Card Reader?")]
		...
		private void OnEntitySpawned(VendingMachine entity)
		{
			NextTick(() =>
			{
				...
				if (_config.Atm.UseCardReader) AttachCardReader(entity);
		...
		{
			_config.Permission = NormalizeBankPermission(_config.Permission);
			_config.Atm.PermissionToOpenATMviaCommand = NormalizeBankPermission(_config.Atm.PermissionToOpenATMviaCommand);
		...
		private bool NormalizeLegacyPermissionsInConfig()
		{
			...
			var normalizedRoot = NormalizeBankPermission(_config.Permission);
			if (!string.Equals(normalizedRoot, _config.Permission, StringComparison.Ordinal))
			{
				_config.Permission = normalizedRoot;
				changed = true;
			}

			var normalizedAtm = NormalizeBankPermission(_config.Atm.PermissionToOpenATMviaCommand);
			if (!string.Equals(normalizedAtm, _config.Atm.PermissionToOpenATMviaCommand, StringComparison.Ordinal))
			{
				_config.Atm.PermissionToOpenATMviaCommand = normalizedAtm;
				changed = true;
			}
		...
		private static string NormalizeBankPermission(string permissionName)
		{
			if (string.IsNullOrEmpty(permissionName))
				return permissionName;

			if (permissionName.StartsWith("nwgbank.", StringComparison.OrdinalIgnoreCase))
				return permissionName;

			if (permissionName.StartsWith("nwg.bank.", StringComparison.OrdinalIgnoreCase))
				return "nwgbank." + permissionName.Substring("nwg.bank.".Length);

			if (permissionName.StartsWith("bank.", StringComparison.OrdinalIgnoreCase))
				return "nwgbank." + permissionName.Substring("bank.".Length);
		...
		private void AttachCardReader(VendingMachine entity)
		{
		...
		{
			if (cardReader.GetParentEntity() != null && cardReader.GetParentEntity().skinID == _config.Atm.Skin)
			{
				OpenATMMenu(player);
				return false;
			}
		...
		private void OpenATMMenu(BasePlayer player)
		{
			// Find existing command or call UI directly
			player.SendConsoleCommand("UI_BankSystem", "open"); // Assumed based on UI_BankSystem command
		}

		[ChatCommand("setatm")]
		private void cmdSetATM(BasePlayer player)
		{
			if (!permission.UserHasPermission(player.UserIDString, "nwgbank.admin") && !player.IsAdmin) return;

			RaycastHit hit;
			if (Physics.Raycast(player.eyes.HeadRay(), out hit, 5f))
			{
				var vending = hit.GetEntity() as VendingMachine;
				if (vending != null)
				{
					vending.skinID = _config.Atm.Skin;
					OnEntitySpawned(vending);
					if (_config.Atm.UseCardReader) AttachCardReader(vending);
					SendReply(player, "Vending machine set to ATM.");
				}
			}
		}
		...
		protected override void LoadDefaultMessages()
		{
			lang.RegisterMessages(new Dictionary<string, string>
			{
				...
				[MainTitle] = "NWG<color=#9fb8ad>Bank</color>",
				[YourBalance] = "Your balance:",
				[CardBankTitle] = "NWGBank",
...
namespace Oxide.Plugins.NWGBankExtensionMethods
{
