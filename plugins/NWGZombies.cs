...
{
    [Info("NWGZombies", "New World Gaming", "1.0.0")]
    public class NWGZombies : RustPlugin, IChaosNPCPlugin
    {
        [PluginReference]
        private Plugin NWGKits, Spawns;

        private static NWGZombies Instance { get; set; }
        ...
        private const string ADMIN_PERMISSION = "nwg.zombies.admin";

        private const string IGNORE_PERMISSION = "nwg.zombies.ignore";
        ...
        {
            ["Notification.BeginSpawn"] = "<color=#9fb8ad>CAUTION!</color> Zombies have been spotted in the area",
            ["Notification.BeginDespawn"] = "<color=#9fb8ad>CAUTION!</color> The zombies appear to be dispersing"
        }, this);
        ...
        private void cmdHorde(BasePlayer player, string command, string[] args)
        {
            ...
            if (args.Length == 0)
            {
                SendReply(player, "/nwgzombies info - Show position and information about active NWG zombies");
                SendReply(player, "/nwgzombies tpto <number> - Teleport to the specified NWG zombies");
                SendReply(player, "/nwgzombies destroy <number> - Destroy the specified NWG zombies");
                SendReply(player, "/nwgzombies create <opt:distance> <opt:profile> - Create a new NWG zombies horde on your position");
                SendReply(player, "/nwgzombies createspawn <opt:membercount> <opt:distance> <opt:profile> - Save your current position as a custom zombie spawn point");
                SendReply(player, "/nwgzombies createloadout - Copy your current inventory to a new zombie loadout");
                SendReply(player, "/nwgzombies hordecount <number> - Set the maximum number of hordes allowed");
                SendReply(player, "/nwgzombies membercount <number> - Set the maximum number of members allowed per horde");
                return;
            }
            ...
            switch (args[0].ToLower())
            {
                case "info":
                    ...
                    {
                        player.SendConsoleCommand("ddraw.text", 30, Color.green, horde.CentralLocation + new Vector3(0, 1.5f, 0), $"<size=20>NWG Zombies {hordeNumber}</size>");
        ...
        private void ccmdHorde(ConsoleSystem.Arg arg)
        {
            ...
            {
                SendReply(arg, "nwgzombies info - Show position and information about active NWG zombies");
                SendReply(arg, "nwgzombies destroy <number> - Destroy the specified NWG zombies");
                SendReply(arg, "nwgzombies create <opt:distance> <opt:profile> - Create a new NWG zombies horde at a random position");
                SendReply(arg, "nwgzombies addloadout <kitname> <opt:otherkitname> <opt:otherkitname> - Convert the specified kit(s) into loadout(s)");
                SendReply(arg, "nwgzombies hordecount <number> - Set the maximum number of hordes allowed");
                SendReply(arg, "nwgzombies membercount <number> - Set the maximum number of members allowed per horde");
                return;
            }
            ...
            switch (arg.Args[0].ToLower())
            {
                ...
                case "addloadout":
                    if (!NWGKits)
                    {
                        SendReply(arg, "Unable to find the NWGKits plugin");
                        return;
                    }
                    ...
                    for (int i = 1; i < arg.Args.Length; i++)
                    {
                        ...
                        object success = NWGKits.Call("GetKitInfo", kitname);
                        if (success == null)
                        {
        ...
        private string GetInfoString()
        {
            if (nextCountTime < Time.time || string.IsNullOrEmpty(cachedString))
            {
                ...
                cachedString = $"There are currently <color=#9fb8ad>{Horde.AllHordes.Count}</color> hordes with a total of <color=#9fb8ad>{memberCount}</color> zombies";
