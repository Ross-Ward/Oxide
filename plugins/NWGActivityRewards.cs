...
{
	[Info("NWGActivityRewards", "New World Gaming", "1.0.0")]
	[Description("Rewards players for in-game activities such as gathering, killing, looting, and fishing.")]
	class NWGActivityRewards : RustPlugin
	{
		[PluginReference]
		private Plugin ImageLibrary, NWGBank, NWGEconomy, AdvancedStatus, ServerPanel;

        #region ~Variables~
		private bool _imgLibIsLoaded = false;
		private bool _statusIsLoaded = false;
		private const string NWGBankBar = "NWGActivityRewards_NWGBank", NWGEconomyBar = "NWGActivityRewards_NWGEconomy", StatusCreateBar = "CreateBar", HttpScheme = "http://", HttpsScheme = "https://";
		...
		private HashSet<string> _defaultEco = new HashSet<string>() { "NWGBank", "NWGEconomy" };
		...
		private readonly Dictionary<string, float> _defaultPermissions = new Dictionary<string, float>() { { "nwg.activityrewards.default", 1f }, { "nwg.activityrewards.vip", 1.1f } };
        private readonly BarSettings _defaultNWGBank = new BarSettings()
        {
            BarID = NWGBankBar,
            Order = 20,
            Height = 26,
            ...
            Image_Local = "NWGActivityRewards_NWGBank",
            Image_Sprite = "",
        ...
        private readonly BarSettings _defaultNWGEconomy = new BarSettings()
        {
            BarID = NWGEconomyBar,
            Order = 20,
            Height = 26,
            ...
            Main_Material = "",
            Image_Url = "https://i.imgur.com/k8jq7yY.png",
            Image_Local = "NWGActivityRewards_NWGEconomy",
            Image_Sprite = "",
            Image_IsRawImage = false,
            Image_Color = "#B9D134",
            Image_Transparency = 1f,
            Text_Key = "MsgNWGEconomy",
            Text_Size = 12,
            Text_Color = "#DAEBAD",
            Text_Font = "RobotoCondensed-Bold.ttf",
            SubText_Size = 12,
		...
		protected override void LoadConfig()
        {
            ...
            if (_config == null || _config.Version == new global::Oxide.Core.VersionNumber())
            {
			...
			if (!_config.BarsList.Any(b => b.BarID == NWGBankBar))
				_config.BarsList.Add(_defaultNWGBank);
			if (!_config.BarsList.Any(b => b.BarID == NWGEconomyBar))
				_config.BarsList.Add(_defaultNWGEconomy);
			SaveConfig();
        }
		
		protected override void SaveConfig() => Config.WriteObject(_config);
		protected override void LoadDefaultConfig() => _config = new Configuration() { EcoPlugins = _defaultEco, PermissionsList = _defaultPermissions, BarsList = new List<BarSettings>() { _defaultNWGBank, _defaultNWGEconomy }, Version = Version };
		#endregion
		...
		private void CheckPlugins(Plugin plugin = null, bool isLoad = false)
		{
			...
			if (plugin == null)
			{
				before = 0;
				_ecoPlugins.Clear();
				if (NWGBank != null && _config.EcoPlugins.Contains(NWGBank.Name) && NWGBank.IsLoaded)
					_ecoPlugins.Add(new EcoPlugin(NWGBank, NWGBankBar));
				if (NWGEconomy != null && _config.EcoPlugins.Contains(NWGEconomy.Name) && NWGEconomy.IsLoaded)
					_ecoPlugins.Add(new EcoPlugin(NWGEconomy, NWGEconomyBar));
			}
		...
		private void InitReward(BasePlayer player, string shortName, string type)
		{
            ...
            {
				if (plugin.Bar.BarID == NWGEconomyBar)
		...
		void OnPluginLoaded(Plugin plugin)
        {
			...
			else if (plugin == NWGBank || plugin == NWGEconomy)
		...
		void OnPluginUnloaded(Plugin plugin)
		{
			...
			else if (plugin.Name == "NWGBank" || plugin.Name == "NWGEconomy")
				CheckPlugins(plugin);
		}
		...
		private const string _gatherPath = "NWGActivityRewards/GatherRewards";
        ...
        private const string _killPath = "NWGActivityRewards/KillRewards";
        ...
        private const string _openPath = "NWGActivityRewards/FirstLootOpenRewards";
        ...
        private const string _pickupPath = "NWGActivityRewards/PickupRewards";
        ...
        private const string _plantingPath = "NWGActivityRewards/PlantingRewards";
        ...
        private const string _fishingPath = "NWGActivityRewards/FishingRewards";
