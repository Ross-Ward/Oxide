...
{
    [Info("NWGParatroopers", "New World Gaming", "1.0.0")]
    [Description("Spawns Paratroopers Event that deploys from the Cargo Plane to random area")]
    class NWGParatroopers : RustPlugin
    {
        [PluginReference] Plugin Kits, Titan;
        private static NWGParatroopers plugin;

        #region Vars
        private static NWGParatroopers ins { get; set; }

        private StoredData storedData;
        private DynamicConfigFile data;

        private const string ADMIN_PERM = "nwgparatroopers.admin";
        private const string PLAYER_PERM = "nwgparatroopers.cancall";
        ...
        private void OnServerInitialized()
        {
            ...
            {
                if (!kvp.Key.StartsWith("nwgparatroopers."))
                {
                    permission.RegisterPermission("nwgparatroopers." + kvp.Key, this);
                }
        ...
        private void OnPlayerDeath(ScientistNPC victim, HitInfo hitInfo)
        {
            ...
            victim.UpdateActiveItem(default(ItemId));
        ...
        private ConfigData GetBaseConfig()
        {
            return new ConfigData
            {
                ...
                {
                    ["nwgparatroopers.cancall.default"] = 120,
                    ["nwgparatroopers.cancall.vip"] = 60
                },
                Version = Version
            };
        }
        ...
        private void UpdateConfigValues()
        {
            ...
            if (configData.Version < new VersionNumber(2, 1, 2))
            {
            ...
            if (configData.Version < new VersionNumber(2, 1, 4))
            {
            ...
            if (configData.Version < new VersionNumber(2, 2, 3))
            {
                configData.npc.WoundedMin = 60;
                configData.npc.WoundedMax = 180;
            }

            if (configData.Version < new VersionNumber(2, 2, 5))
            {
            ...
            if (configData.Version < new VersionNumber(2, 2, 6))
            {
            ...
            if (configData.Version < new VersionNumber(2, 2, 8))
            {
            ...
            if (configData.Version < new VersionNumber(2, 3, 0))
