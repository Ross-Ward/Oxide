...
{
	[Info("NWGExtractionRareMinerals", "New World Gaming", "1.0.0")]
	[Description("Rare mineral drops from resource extraction, with smelting and recycling support")]
	public class NWGExtractionRareMinerals : RustPlugin
	{
		#region Version
		class PluginVersion
		{
		...
		private readonly string usePermission = "nwgextractionrareminerals.use";
		private readonly string smeltingPermission = "nwgextractionrareminerals.allowsmelting";
		private readonly string recyclerPermission = "nwgextractionrareminerals.allowrecycler";
		#endregion
		...
		private void InitBaseItem()
		{
			...
			// cookable.ModInit(itemInfo); // Potentially unnecessary or has different signature in this environment
		...
		private void InitCookableItems()
		{
			foreach (var entity in BaseNetworkable.serverEntities)
			{
                ...
                int itemId = GetItemId(config.ItemShortName);
				List<Item> foundItems = container.inventory.itemList.Where(x => x.info.itemid == itemId).ToList();
