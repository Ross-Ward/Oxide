...
{
    [Info("NWGKitController", "New World Gaming", "1.0.0")]
    [Description("A new, modern kit system for Rust")]
    public class NWGKitController : RustPlugin
    {
        ...
        protected override void LoadDefaultMessages()
        {
            ...
            {
                ["ChatTitle"] = "<color=#91b9fa>[ NWG ]:</color>",
                ["ButtonView"] = "VIEW",
        ...
        {
            if (!permission.UserHasPermission(player.UserIDString, "nwgkitcontroller.admin"))
            {
        ...
        private void ConvertRustKitsData(BasePlayer player = null)
        {
            try
            {
                ...
                foreach (var kit in rustKitsStoredData["_kits"].Values)
                {
                    ...
                    {
                        KitName = kit.Name,
                        Permission = kit.RequiredPermission.Contains('.') ? "nwgkitcontroller" + kit.RequiredPermission.Substring(5) : $"nwgkitcontroller.{kit.RequiredPermission}",
            ...
            switch (arg.Args[0])
            {
                ...
                case "permission":
                    if (arg.Args.Length > 1 && !string.IsNullOrEmpty(arg.Args[1]))
                    {
                        ...
                        if (!permission.Contains("nwgkitcontroller.")) permission = "nwgkitcontroller." + permission;
        ...
        private void RegisterCommandsAndPermissions(bool reRegister = false)
        {
            ...
            foreach (var kit in _kitData)
                ...
                {
                    if (!kit.Permission.Contains("nwgkitcontroller.")) kit.Permission = "nwgkitcontroller." + kit.Permission;
                    if (!permission.PermissionExists(kit.Permission, this)) permission.RegisterPermission(kit.Permission, this);
                }

            permission.RegisterPermission("nwgkitcontroller.admin", this);
        }
        ...
        ClaimKitResponse ClaimKit(BasePlayer player, string kitName, bool fromAdmin = false, bool fromSpawn = false)
        {
            ...
            var isAdmin = permission.UserHasPermission(player.UserIDString, "nwgkitcontroller.admin");
        ...
        {
            var kitList = Interface.GetMod().DataFileSystem.ReadObject<List<aKit>>($"NWGKitController{Path.DirectorySeparatorChar}kitData");
        ...
        {
            var userData = Interface.GetMod().DataFileSystem.ReadObject<List<PlayerUsedKits>>($"NWGKitController{Path.DirectorySeparatorChar}userData");
        ...
        {
            Interface.GetMod().DataFileSystem.WriteObject($"NWGKitController{Path.DirectorySeparatorChar}userData", _userData);
        }
