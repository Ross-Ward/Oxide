...
using Oxide.Plugins.NWGCraftsExtensionMethods;
using Rust;
using UnityEngine;
...
{
	[Info("NWGCrafts", "New World Gaming", "1.0.0")]
	public class NWGCrafts : RustPlugin
	{
		#region Fields
		...
		private static NWGCrafts _instance;
		...
		private const string
			Layer = "NWG.Crafts",
			EditLayer = "NWG.Crafts.Edit",
			PermEdit = "nwgcrafts.admin",
			PermSetWb = "nwgcrafts.setworkbench",
			PermBypass = "nwgcrafts.bypass";
		...
		private class Configuration
		{
			...
			public string[] Commands = {"ncrafts", "nwg.crafts"};
		...
		private void CmdConsoleCrafts(ConsoleSystem.Arg arg)
		{
			...
			switch (arg.Args[0])
			{
				...
				case "save_edit":
				{
					...
					craft.Permission = NormalizeCraftsPermission((string) values["Permission"]);
		...
		private bool NormalizeLegacyPermissionsInConfig()
		{
			...
			var normalizedRoot = NormalizeCraftsPermission(_config.Permission);
			...
			if (_config.Categories != null)
			{
				...
				{
					var normalizedCategory = NormalizeCraftsPermission(category.Permission);
					if (!string.Equals(normalizedCategory, category.Permission, StringComparison.Ordinal))
					{
						category.Permission = normalizedCategory;
						changed = true;
					}

					if (category.Crafts == null) continue;
					foreach (var craft in category.Crafts)
					{
						var normalizedCraft = NormalizeCraftsPermission(craft.Permission);
						if (!string.Equals(normalizedCraft, craft.Permission, StringComparison.Ordinal))
						{
							craft.Permission = normalizedCraft;
							changed = true;
						}
					}
				}
			...
			if (_config.CustomWorkbench != null)
			{
				foreach (var entry in _config.CustomWorkbench.Values)
				{
					if (entry?.WorkBench == null || entry.WorkBench.Count == 0)
						continue;

					var rebuilt = new Dictionary<string, int>();
					foreach (var wb in entry.WorkBench)
					{
						var normalizedKey = NormalizeCraftsPermission(wb.Key);
						if (!string.Equals(normalizedKey, wb.Key, StringComparison.Ordinal))
							changed = true;

						rebuilt[normalizedKey] = wb.Value;
					}

					entry.WorkBench = rebuilt;
				}
			}
		...
		private static string NormalizeCraftsPermission(string permissionName)
		{
			if (string.IsNullOrEmpty(permissionName))
				return permissionName;

			if (permissionName.StartsWith("nwgcrafts.", StringComparison.OrdinalIgnoreCase))
				return permissionName;

			if (permissionName.StartsWith("nwg.crafts.", StringComparison.OrdinalIgnoreCase))
				return "nwgcrafts." + permissionName.Substring("nwg.crafts.".Length);

			if (permissionName.StartsWith("crafts.", StringComparison.OrdinalIgnoreCase))
				return "nwgcrafts." + permissionName.Substring("crafts.".Length);
		...
		protected override void LoadDefaultMessages()
		{
			lang.RegisterMessages(new Dictionary<string, string>
			{
				...
				[CraftsDescription] = "The NWG Crafts system provided by New World Gaming allows for custom crafting with categorization and level-based unlocks.",
				...
				["Permission"] = "Permission (ex: nwgcrafts.vip)",
			...
			lang.RegisterMessages(new Dictionary<string, string>
			{
				...
				[CraftsDescription] = "Система NWG Crafts от New World Gaming позволяет создавать кастомные предметы с распределением по категориям и уровням.",
				...
				["Permission"] = "Разрешение (пример: nwgcrafts.vip)",
...
namespace Oxide.Plugins.NWGCraftsExtensionMethods
{
