#define USE_FIXES
// Shop front fixes (not compatible with Stacks Extended!)
using System;
...
{
    [Info("NWGTrade", "New World Gaming", "1.0.0")]
    public class NWGTrade : RustPlugin
    {
        #region Fields

        private static NWGTrade _instance;
        ...
        private const string PermAllowUse = "nwgtrade.use";
        ...
        private class Cfg
        {
            ...
            public static Cfg DefaultConfiguration() => new Cfg
            {
                ...
                Permissions = new List<PermissionDefinition>
                {
                    ...
                    new PermissionDefinition{Order = 2, Name = "vip", Cooldown = 10f, MaxDist = 0f, TradingSlots = 12}
                },
            ...
            private void OnDeserialized(StreamingContext _)
            {
                if (Permissions == null || Permissions.Count == 0)
                {
                    ...
                    _instance.NextTick(()=>Interface.Oxide.UnloadPlugin(nameof(NWGTrade)));
                    return;
                }
                if(Permissions.Count == 1)
                    return;
                Permissions.Sort((a, b) => a.Order.CompareTo(b.Order));
                for (var i = 1; i < Permissions.Count; i++)
                {
                    Permissions[i].PermName = $"nwgtrade.{Permissions[i].Name}";
        ...
        private void CmdChatTrade(BasePlayer player, string command, string[] args)
        {
            ...
            if (!PendingTrade.HasPendingTrade(player))
            {
        ...
        {
            var highestOrderPerm = _cfg.Permissions[0];
        ...
        protected override void LoadDefaultMessages()
        {
            lang.RegisterMessages(new Dictionary<string, string>
            {
                ...
                { "Msg.TradeSuccessful", "Your trade with <color=#81B67A>{0}</color> was successful!" },
                { "Msg.TradeTimeoutVendor", "<color=#81B67A>{0}</color> didn't respond to your trade request." },
                { "Msg.TradeTimeoutCustomer", "You haven't responded to <color=#81B67A>{0}</color>'s trade request." },
                ...
                { "Error.NoSuchPlayer", "No such player found or they are offline." },
                { "Error.MultiplePlayers", "Found multiple players with this name!\nRefine your search or use SteamID." },
                { "Error.SelfTrade", "You can't trade with yourself!" },
