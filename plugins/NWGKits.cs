...
using Oxide.Plugins.NWGKitsExtensionMethods;
using UnityEngine;
...
{
	[Info("NWGKits", "New World Gaming", "1.0.0")]
	public class NWGKits : RustPlugin
	{
		...
		private static NWGKits _instance;

		private const string
			PERM_ADMIN = "nwgkits.admin",
		...
		private class Configuration
		{
			...
			public string ChangeAutoKitPermission = "nwgkits.changeautokit";
		...
		private void CmdKitsConsole(ConsoleSystem.Arg arg)
		{
			...
			switch (arg.Args[0])
			{
				...
				case "saveitem":
				{
					...
					if (!_itemEditing.TryGetValue(player.userID, out var editing) || editing == null)
						return;
				...
				case "savekit":
				{
					...
					if (!_kitEditing.TryGetValue(player.userID, out var editing) || editing == null)
						return;
		...
		private void RegisterPermissions()
		{
			...
			_config.ChangeAutoKitPermission = NormalizeKitsPermission(_config.ChangeAutoKitPermission);
			...
			var kitsChanged = false;

			_data.Kits.ForEach(kit =>
			{
				var perm = NormalizeKitsPermission(kit.Permission);
				if (!string.Equals(perm, kit.Permission, StringComparison.Ordinal))
				{
					kit.Permission = perm;
					kitsChanged = true;
				}
		...
		{
			var normalized = NormalizeKitsPermission(_config.ChangeAutoKitPermission);
			if (string.Equals(normalized, _config.ChangeAutoKitPermission, StringComparison.Ordinal))
				return false;

			_config.ChangeAutoKitPermission = normalized;
			return true;
		}

		private static string NormalizeKitsPermission(string permissionName)
		{
			if (string.IsNullOrEmpty(permissionName))
				return permissionName;

			if (permissionName.StartsWith("nwgkits.", StringComparison.OrdinalIgnoreCase))
				return permissionName;

			if (permissionName.StartsWith("nwg.kits.", StringComparison.OrdinalIgnoreCase))
				return "nwgkits." + permissionName.Substring("nwg.kits.".Length);

			if (permissionName.StartsWith("kits.", StringComparison.OrdinalIgnoreCase))
				return "nwgkits." + permissionName.Substring("kits.".Length);
...
namespace Oxide.Plugins.NWGKitsExtensionMethods
{
